    ...
    def get_observation(self) -> Observation:
        ...
        for camera_name, camera_config in self._obs_config.camera_configs.items():            
            ...
            if camera_config.mask and camera_config.masks_as_one_channel:
                mask_data, mask_id_to_name = get_mask(self.camera_sensors_mask[camera_name], rgb_handles_to_mask)
            elif camera_config.mask:
                mask_data, mask_id_to_name = get_mask(self.camera_sensors_mask[camera_name], lambda x: x)
            else:
                mask_data, mask_id_to_name = (None, None)
            ...
            temp_mask_id_to_name_dict[f'{camera_name}_mask_id_to_name'] = mask_id_to_name
    ...
    def execute_waypoints_bimanual(self, do_record) -> bool:
        ...
        while True:
            ...
            for i, (right_point, left_point) in enumerate(zip(right_waypoints, left_waypoints)):
                ...
                # note that a grasp is also a collision, but a grasped object apparently becomes part of the arm "collection of objects" in the simulator
                ...
                for s in self.pyrep.get_objects_in_tree(object_type=ObjectType.SHAPE):
                    if s in grasped_objects:
                        continue
                    # if s in self._robot_shapes[0] or s in self._robot_shapes[1]:
                    #     continue
                    ...
                    if hasattr(self, 'no_self_collisions') and self.no_self_collisions:
                        ...
                        for obj in all_objects: # first check if grippers are colliding with the other arm
                            if "rightarm" in obj.get_name().lower():
                                collision = self.robot.left_gripper.check_collision(obj)
                            elif "leftarm" in obj.get_name().lower():
                                collision = self.robot.right_gripper.check_collision(obj)
                            if collision:
                                raise DemoError('Arm-to-arm collision detected.', self.task)
                        if self.robot.is_in_collision(): # then check if the body of an arm is colliding with the other arm
                            for obj in all_objects:
                                if "rightarm" in obj.get_name().lower():
                                    collision = self.robot.left_arm.check_arm_collision(obj)
                                elif "leftarm" in obj.get_name().lower():
                                    collision = self.robot.right_arm.check_arm_collision(obj)
                                else:
                                    collision = self.robot.left_arm.check_arm_collision(obj) or self.robot.right_arm.check_arm_collision(obj)
                                if collision:
                                    # print("-----------COLLISION WITH " + obj.get_name() + "-------------")
                                    if "panda" in obj.get_name().lower():
                                        if self.task.name == "bimanual_pick_plate": # bugfix for this task: detects an arm-to-arm collision when grasping the plate
