...
class BimanualCloseJar(BimanualTask):
    ...
    def init_episode(self, index: int) -> List[str]:
        ...
        jar0 = Shape('jar0')
        jar0.set_orientation([0, 0, 0], reset_dynamics=False)
        w4 = Dummy('waypoint4')
        w4.set_position([0.15, 0.0, 0.0], relative_to=self.jars[0],
                        reset_dynamics=False)
        w5 = Dummy('waypoint5')
        w5.set_position([0.035, 0.0, 0.0], relative_to=self.jars[0],
                        reset_dynamics=False)

        w12 = Dummy('waypoint12')
        w12.set_orientation([-np.pi, 0, -np.pi], reset_dynamics=False)
        w12.set_position([0.0, 0.0, 0.17], relative_to=self.jars[0],
                        reset_dynamics=False)
        ...
        hold_jar = ProximitySensor('hold_jar')
        self.conditions += [DetectedCondition(self.lid, success)] #, DetectedCondition(self.robot.right_arm.get_tip(), hold_jar)]
