...
def _extract_obs_bimanual(obs: BimanualObservation, channels_last: bool, observation_config: ObservationConfig):
    ...
    for camera_name, config in observation_config.camera_configs.items():
        ...
        if config.mask:
            obs_dict['%s_mask_id_to_name' % camera_name] = obs.misc['%s_mask_id_to_name' % camera_name] # add mapping between mask and object id
...
def _extract_obs_unimanual(obs: Observation, channels_last: bool, observation_config):
    ...
    for config, name in [
        ...
        if config.mask:
            obs_dict['%s_mask_id_to_name' % name] = obs.misc['%s_mask_id_to_name' % name] # add mapping between mask and object id
