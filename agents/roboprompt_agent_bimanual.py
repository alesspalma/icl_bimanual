...
class RoboPromptAgentBimanual(LLMTrackingMixin, Agent):
    """One agent that handles the bimanual robot as a single entity. The action space is a 14-dimensional vector."""
    ...
    def _preprocess(self, obs, step, **kwargs):
        ...
        if len(self.actions) == 0:
            user_prompt = self.handler.get_user_prompt(mask_dict, mask_id_to_sim_name, point_cloud_dict, self)
    ...
    def act(self, step: int, observation: dict,
        ...
        if len(self.actions) == 0:
            output = self._postprocess(output_text) # extract continuous actions from the LLM prediction
            self.actions = output
