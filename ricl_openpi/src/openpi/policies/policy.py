...
class RiclPolicy(BasePolicy):
    ...
    def retrieve(self, obs: dict) -> dict:
        ...
        if self._use_action_interpolation:
            ...
            raw_query_dist = float(np.linalg.norm(query_embedding - first_embedding))
            distances.append(raw_query_dist)
            print(f'[RICL] raw distances (before clip/norm): {[round(d,2) for d in distances]}, max_dist={self._max_dist:.2f}, query_norm={float(np.linalg.norm(query_embedding)):.2f}, retrieved0_norm={float(np.linalg.norm(first_embedding)):.2f}')
    ...
    @override
    def infer(self, obs: dict, debug: bool = False) -> dict:  # type: ignore[misc]
