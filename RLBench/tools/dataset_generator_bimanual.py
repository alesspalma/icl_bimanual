...
def run_all_variations(task_name, headless, save_path, episodes_per_task, image_size, no_self_collisions):
    ...
    for task in tasks:
        ...
        for ex_idx in range(episodes_per_task):
            ...
            while attempts > 0:
                try:
                    variation = 0 if task_name == "bimanual_put_item_in_drawer" else np.random.randint(possible_variations)
                    ...
                    task_env._scene.no_self_collisions = no_self_collisions

                    # TODO: for now we do the explicit looping.
                    demo, = task_env.get_demos(amount=1, live_demos=True)
                #  NoWaypointsError, DemoError,
                except (BoundaryError, WaypointError, InvalidActionError, TaskEnvironmentError, RuntimeError) as e:
...
def main(save_path, tasks, episodes_per_task, no_self_collisions, all_variations, headless, image_size):
    ...
    if no_self_collisions:
        print("NO SELF COLLISIONS WILL BE RECORDED")
    ...
    fn = partial(run_all_variations, headless=headless, save_path=save_path, episodes_per_task=episodes_per_task, image_size=image_size, no_self_collisions=no_self_collisions)
